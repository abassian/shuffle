// shuffle-js imports
const { EVMLC, Account, Contract } = require('shuffle-core');
const { Keystore } = require('shuffle-keystore');
	...
	.then(() =>
		explain(
			'We created an EVM transaction to send 500 wei from node1 to node2. The \n' +
				...
				'The client-facing service running in Shuffle relayed the transaction to Huron \n' +
				'for consensus ordering. Huron gossiped the raw transaction to the other Huron \n' +
				'nodes which ran it through the consensus algorithm before committing it back to \n' +
				'Shuffle as part of Block. So each node received and processed the transaction. \n' +
	...
	.then(() =>
		explain(
			"We created an EVM transaction to call the 'contribute' method of the SmartContract. \n" +
				...
				'As an exercise you can check that the transaction was run through every Huron \n' +
