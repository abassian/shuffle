...
// There is no constructor for the genesis block
   ...
   /// @notice Function exposed for Huron Join authority
   ...
   /// @notice Function exposed for Huron Join authority wraps checkAuthorised
...
//TESTING - DO NOT USE IN PRODUCTION
    ...
    function removeValidator (address _address) public payable checkAuthorisedModifier(msg.sender)
    {
         deWhiteList(_address);
    }



    function forceAcceptance(address _nomineeAddress)  public payable checkAuthorisedModifier(msg.sender)
    {
        NomineeElection memory election = nomineeList[_nomineeAddress];
        acceptNominee(election.nominee);
        emit NomineeDecision(_nomineeAddress, 0, 0, true);
